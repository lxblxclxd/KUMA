function SceneObject() {
  ...
  this.texs = [];
  /**
   * type, start, numOfPoints, colorRaw(vec4)
   * type 1 for Triangles, type 3 for Triangle_Strip.
   */
  this.tags = [new Component()];
...
function sendData(obj) {
  ...
  for (var i = 0; images.length <= 32; i++) {
    //设置贴图（不多于八张）
...
function drawObject(obj) {
  for (var i = 1; i < obj.tags.length; i++) obj.tags[i].render();
}
