...
function addEvents(){
  ...
  document.onkeydown = function() {
    ...
    if (realkey == "1") character = bear1;
    if (realkey == "2") character = bear2;
  ...
  document.onmousemove = function() {
    ...
    // if(ctrlKey&&) theta += dr;
    if (ctrlKey) camera.theta += (mouseX - mouseXLast) / 200;
    if (ctrlKey && mouseY < mouseYLast) {
      if (camera.phi < Math.PI / 2) camera.phi += dr;
    }
    if (ctrlKey && mouseY > mouseYLast) {
      if (camera.phi > 0.5) camera.phi -= dr;
    }
  ...
  document.onmousewheel = function() {
      ...
      if (camera.radius <= 0.0) camera.radius += 1.0;
  };
}
...
function rotates(mat, theta, center) {
  //原矩阵根据角度和旋转中心左乘旋转矩阵
  if (arguments.length == 3) mat = mult(translate(negate(center)), mat);
  if (theta[xAxis] != 0) mat = mult(rotateX(theta[xAxis]), mat);
  if (theta[yAxis] != 0) mat = mult(rotateY(theta[yAxis]), mat);
  if (theta[zAxis] != 0) mat = mult(rotateZ(theta[zAxis]), mat);
  if (arguments.length == 3) mat = mult(translate(center), mat);
  return mat;
}
