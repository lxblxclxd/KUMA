function Component(type, start, numOfPoints, material) {
  ...
  this.calRMat = function() {
    if (this.father == null) return rotates(mat4(), this.theta);
    else return rotates(father.calRMat(), this.theta);
  }
  this.calCMT = function() {
    if (this.father == null) return rotates(mat4(), this.theta, this.rootPos);
    else return rotates(this.father.calCMT(), this.theta, this.rootPos);
  }
...
function ellipsoid(points, normals, texs, tags, x, y, z, a, b, c, material) { //椭球
  ...
  for (var j = 0; j < 180; j += precise) {
    ...
    for (var i = 0; i <= 360; i += precise) {
      ...
      points.push(
        vec3(x + a * rx, y + b * ry, z + c * rz),
        vec3(x + a * rxd, y + b * ryd, z + c * rzd)
      );
      normals.push(
        vec3(rx / a, ry / b, rz / c),
        vec3(rxd / a, ryd / b, rzd / c)
      );

      var texx, texy, texxd, texyd;
      texy = 0.5 + ry / 2;
      texyd = 0.5 + ryd / 2;
      if (rz >= 0) texx = rx / 2 + 0.5;
      else texx = 1.5 - rx / 2;
      if (rzd >= 0) texxd = rxd / 2 + 0.5;
      else texxd = 1.5 - rxd / 2;
      if (j == 91) {
        //解决贴图边界的错误问题
        texx = texxd;
        texy = texyd;
